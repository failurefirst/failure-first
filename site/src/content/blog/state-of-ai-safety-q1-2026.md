...
draft: false
image: "https://cdn.failurefirst.org/images/blog/state-of-ai-safety-q1-2026.png"
...
**The abliterated model finding** provides additional evidence. In the Qwen3.5 obliteratus series (models with safety training deliberately removed), compliance declines monotonically across scale -- 99.8% at 0.8B, 94.8% at 1.9B, 78.3% at 4.2B, and 54.2% at 9.0B (9B n=2,019). The honest rank trend is rho=-1.0 over four scale points, exact permutation p=0.083 -- not statistically significant, because a four-point monotone series floors at 2/24=0.083 and cannot reach p<0.05. The large per-scale n buys tight, non-overlapping per-scale Wilson 95% confidence intervals (which carry the trend), not trend power. (An earlier "Spearman rho=-0.949, p=0.051" figure was a mislabeled Pearson-on-log and has been corrected.) Safety-like behaviour partially re-emerges at scale even when explicitly removed -- as textual hedging, not refusal (9B non-compliance is 924/2,019 PARTIAL, zero outright refusals). This suggests some safety properties are emergent rather than solely trained.
