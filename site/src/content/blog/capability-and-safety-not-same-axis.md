...
audio: "https://cdn.failurefirst.org/audio/blog/capability-and-safety-not-same-axis.m4a"
---
...
The decline is monotone and the per-scale Wilson 95% confidence intervals do not overlap between adjacent scales; with only four model sizes a rank-trend test is structurally underpowered (it cannot reach significance), so the confidence intervals -- not a p-value -- carry the trend. At 9.0B parameters, the abliterated model -- a model with safety training explicitly removed -- produces safety-adjacent (hedged) behaviour in roughly 46% of cases. It adds disclaimers, hedges, and caveats. It never actually refuses (broad ASR remains ~100%), but it increasingly sounds like it is considering refusal.
