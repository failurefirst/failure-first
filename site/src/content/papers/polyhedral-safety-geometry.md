...
Table [4](#tab:reemergence) presents the re-emergence curve: the relationship between model scale and residual safety after single-direction abliteration.
  ...
  **Model**                      $n$   **COMPL.**   **PARTIAL**   **REFUSAL**   **Strict ASR**   **Broad ASR**
  ...
  : OBLITERATUS Qwen3.5 abliterated series (post-#810-recovery curve, 9B $n=2{,}019$). Single-direction abliteration achieves near-complete safety suppression at 0.8B but safety-like behavior re-emerges at scale. Strict ASR counts only COMPLIANCE verdicts; Broad ASR counts COMPLIANCE + PARTIAL. Verdict source is the OBLITERATUS import classifier (`obliteratus-import`), not LLM-judge. The 9B figure was 47.3% at $n=317$ before the basename-collision recovery re-exposed masked results; it is superseded.

The strict ASR decline from 99.8% to 54.2% is the central empirical finding. At 0.8B parameters, the model has insufficient representational capacity in its residual safety dimensions to produce safety behavior after the primary refusal direction is removed. At 9.0B, the residual ~3 safety dimensions become expressive enough to reconstruct safety-like hedging — the 9B non-compliance is entirely PARTIAL/hedging (924/2,019), with zero outright refusals, so this is hedging re-emergence, not refusal re-emergence.
...
**Prediction:** Complete abliteration requires identifying and removing all ~4 category-specific refusal directions independently. The near-orthogonality between directions ($c_{jk} \in [0.017, 0.247]$) means each must be targeted separately. Multi-direction abliteration has not been systematically studied.
