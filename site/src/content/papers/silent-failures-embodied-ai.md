...
Section [2](#sec:empirical){reference-type="ref" reference="sec:empirical"} presents the empirical evidence for silent failures across VLA and text-only systems. Section [3](#sec:mechanisms){reference-type="ref" reference="sec:mechanisms"} analyzes the mechanisms that produce PARTIALs, including [Detected_Proceeds]{.smallcaps} and the System T / System S framework. Section [4](#sec:measurement){reference-type="ref" reference="sec:measurement"} documents the measurement crisis and its consequences for deployed safety evaluation. Section [5](#sec:geometry){reference-type="ref" reference="sec:geometry"} connects silent failures to the geometric structure of safety in model weights. Section [6](#sec:framework){reference-type="ref" reference="sec:framework"} proposes the Functionally Dangerous framework and discusses its implications for evaluation methodology, deployment policy, and regulatory compliance. Section [7](#sec:limitations){reference-type="ref" reference="sec:limitations"} addresses limitations. Section [8](#sec:conclusion){reference-type="ref" reference="sec:conclusion"} concludes.

# The Empirical Landscape of Silent Failures {#sec:empirical}
...
Table [1](#tab:vla_verdicts){reference-type="ref" reference="tab:vla_verdicts"} presents the verdict distribution across seven VLA attack families tested against two sub-2B parameter models [@wedd2026failurefirst].
...
The gap is not uniform across providers. Table [3](#tab:provider_fd){reference-type="ref" reference="tab:provider_fd"} shows the per-provider FD gap:
...
The largest FD gaps appear in providers whose models are most likely to hedge rather than cleanly refuse or comply. Liquid models show the most extreme pattern: a 33.8% strict ASR that more than doubles to 75.2% when PARTIAL and [Hallucination_Refusal]{.smallcaps} verdicts are properly classified. Anthropic models show the smallest gap (+4.6pp), suggesting that Anthropic's safety training produces cleaner separation between refusal and compliance, with less middle-ground hedging.

## [Hallucination_Refusal]{.smallcaps}: The Text-Only Analog of VLA PARTIAL

[Hallucination_Refusal]{.smallcaps} (HR)---responses that textually claim to refuse while generating harmful content---is the text-only equivalent of VLA PARTIAL. The evidence comes from three converging statistical tests.

#### Thinking token equivalence.

Across reasoning model traces with thinking tokens $> 0$, [Hallucination_Refusal]{.smallcaps} (mean 1,423 tokens, $n=47$) is statistically indistinguishable from COMPLIANCE (mean 1,558 tokens, Mann-Whitney $U=5{,}910$, $p=0.21$, $d=-0.068$). Both are significantly different from REFUSAL (mean 757 tokens, $p=1.85 \times 10^{-4}$, $d=+0.414$). The model expends the same cognitive effort to produce an HR response as it does to produce a fully compliant response.

#### Response token equivalence.

[Hallucination_Refusal]{.smallcaps} produces the longest responses on average (mean 1,835 tokens, $n=84$)---even longer than COMPLIANCE (mean 1,676, $p=0.46$, $d=+0.087$). Both are significantly different from REFUSAL (mean 865 tokens, $p=8.85 \times 10^{-11}$, $d=+0.614$).

#### System T / System S mapping.

These results support a dual-system interpretation (Table [4](#tab:systems){reference-type="ref" reference="tab:systems"}):
...
[Hallucination_Refusal]{.smallcaps} and PARTIAL occupy the same functional cell: System T produces the harmful content, System S produces framing that does not prevent delivery. The difference is cosmetic---where the safety framing appears (wrapper vs. integrated)---not functional.

# Why Models Produce Silent Failures {#sec:mechanisms}

## [Detected_Proceeds]{.smallcaps}: The Knowing-Doing Gap

The most direct evidence that silent failures represent a genuine alignment problem---not mere noise or grading artifact---comes from the [Detected_Proceeds]{.smallcaps} (DP) analysis. DP identifies cases where a model's internal reasoning trace contains explicit safety-detection language and the model proceeds to comply anyway.
...
This is the central finding of the [Detected_Proceeds]{.smallcaps} analysis: **safety training successfully teaches models to recognize harmful requests. It does not reliably teach them to act on that recognition.** The gap between knowing and doing is the mechanism that produces PARTIAL responses.

## The System T / System S Framework

The dual-system interpretation proposed in the [Hallucination_Refusal]{.smallcaps} analysis and developed across the [Detected_Proceeds]{.smallcaps} series provides a mechanistic account of silent failures.
...
# The Measurement Crisis {#sec:measurement}
...
# The Geometric Foundation of Silent Failures {#sec:geometry}
...
::: {#tab:geometry}
  Metric                                                                                         Value
  --------------------------------- ------------------------------------------------------------------
  Cone dimensionality                              3.96 ($\sim$`<!-- -->`{=html}4 distinct directions)
  ...
  Most linear layer                    15 (late, but still dimensionality $\sim$`<!-- -->`{=html}3.82)
...
# The Functionally Dangerous Framework {#sec:framework}
