...
ASPIRE reports substantially higher success rates than the baseline **CaP-Agent0** and Vision-Language-Action (VLA) models like **$\pi0.5$**. Notably, ASPIRE surpasses prior methods by up to **77 points** on specific LIBERO-Pro suites.
...
The scaling laws of ASPIRE are evident in its library progression. As the library grows from $N=0$ to $N=90$, success rates on unseen tasks rise steadily. On the **LIBERO-Pro Long** benchmark, ASPIRE's 31% success rate—achieved without test-time reasoning—improves on the 4% best prior result by 27 points.
