...
2. **NotebookLM's slide-deck Studio generation has a reproducible content-sensitive pre-generation filter.** Four notebooks across three content classes triggered the same fast-fail profile (~20-90 seconds, terminal `failed` status before generation begins). The filter is not chemistry-specific (empirical AI safety research also triggers it) and not source-count-driven (a 60-source ablation failed identically to the 11-source original). The precise content discriminator remains uncharacterized at n=1 per cell.
...
`nlm slides create` was submitted once against the copy. The artifact reached terminal `failed` status within approximately 20 seconds — actually faster than the 60-90 second failures observed on the original LSD and Gemma #349 notebooks. **Source count is not the driver.** The content axis is, but the precise discriminator remains uncharacterized at the current experimental depth.

### 3.3 The current picture

Four pre-generation fast-fails across three content-distinct notebooks (LSD original, LSD re-test, LSD+60 copy, Gemma #349). Two successful slide-deck generations (F1 Novel Attack, F1 Regulatory). One late-stage failure in a mechanically distinct mode (F1 Defense). The filter is real and reproducible. It is not chemistry-specific, not source-count-driven, and not simply "AI safety research." Characterizing the exact content discriminator would require additional experiments.
...
*Classification: Research — Empirical Pilot | Status: Draft*
*Date: 2026-04-11*
